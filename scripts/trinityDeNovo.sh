#!/usr/bin/env bash
# trinityDeNovo.sh
# Usage: sbatch scripts/trinityDeNovo.sh 1>results/logs/trinity_de_novo.log 2>results/logs/trinity_de_novo.err


# Get the list of left reads and store as $leftReads
leftReads="$(ls -q data/paired/Aip*.R1.fastq)"
...
rightReads="$(ls -q data/paired/Aip*.R2.fastq)"
